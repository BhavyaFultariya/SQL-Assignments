...
(77,'Original Frankfurter',12,2,'12 boxes',13,32,0,15,0)

/*Query-1:
Write a query to get a Product list (id, name, unit price) where current products cost
less than $20.*/

SELECT ProductID,ProductName,UnitPrice FROM Products WHERE (UnitPrice < 20) AND (Discontinued=0);

/*Query-2:
Write a query to get Product list (id, name, unit price) where products cost between
$15 and $25.*/
...
SELECT Top 1 
(SELECT COUNT(Discontinued) AS CurrentProducts FROM Products GROUP BY Discontinued HAVING Discontinued=0) AS CurrentProducts, 
(SELECT COUNT(Discontinued) AS Discontinued FROM Products GROUP BY Discontinued HAVING Discontinued=1) AS Discontinued 
FROM Products 

/*Query-6:
Write a query to get Product list (name, units on order , units in stock) of stock is less
